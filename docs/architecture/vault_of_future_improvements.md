...
This roadmap is organized into two main areas:

1. **Open roadmap priorities** — items still pending or planned, ordered by recommended implementation sequence.
2. **Implemented and partially implemented items** — completed baselines and historical improvements, kept at the end for traceability.

---

# Open Roadmap Priorities


# 1. GitHub Repository and DevOps Hygiene — Remaining Items

## Status

🔄 Partially implemented — governance templates completed; CI and LocalStack compose still open.
...
The items below close the remaining gaps for **standard GitHub practice**, **repeatable local S3**, and **automated dbt validation** on pull requests.

## Remaining improvements

| Priority | Item | Status | Rationale |
|----------|------|---------|-----------|
| High | **CI (GitHub Actions):** `dbt deps`, `dbt parse`, `dbt build --select staging.anatel+` | ⬜ Pending | Prevent regressions on PRs; aligns with dbt CI row |
| Medium | **`docker-compose` for LocalStack** | ⬜ Pending | Reproducible S3 (`anatel-lake`) for staging sources and `upload_parquet_to_s3` without manual setup |
...
# 2. dbt Governance — Next Improvements
...
# 4. Incremental Processing Control
...
# 6. Knowledge Base and Learnings Documentation
...
# 7. Native Spark Parquet Writing
...
# 8. Physical Partitioning Strategy
...
# 12. Cloud and Data Lake Evolution
...
# 13. Future Migration to Lakehouse Standards
...
# Implemented and Partially Implemented Items


# 14. Remove Temporary CSV Layer
...
# 15. Pipeline Entrypoint and Execution Organization
...
The project now has an orchestration layer and a general project entrypoint for coordinating the main execution flow.
...
Current implemented execution flow:
...
The current implementation already centralizes the main pipeline flow, but may still evolve into dedicated pipeline runners as the project grows.
...
Although functional, the previous entrypoint did not represent the complete project workflow and could become difficult to maintain as new processes were added.
...
# 16. Migration to Analytics Engineering Layer
...
# 17. Medallion Architecture Evolution
...
# 19. GitHub Repository Governance — Completed Items
...
| Priority | Item | Status | Rationale |
|----------|------|---------|-----------|
| Medium | ~~**`.github/pull_request_template.md`**~~ | ✅ Done | Standardized PR descriptions (Summary / Motivation / Impact / Validation) aligned with `CONTRIBUTING.md` |
| Medium | ~~**`.github/ISSUE_TEMPLATE/bug_report.md`**~~ | ✅ Done | Standardized bug reporting with reproducibility, environment and validation evidence |
| Medium | ~~**`.github/ISSUE_TEMPLATE/feature_request.md`**~~ | ✅ Done | Standardized feature proposals with motivation, impact and alternatives |
| Low | ~~**`CODE_OF_CONDUCT.md`**~~ | ✅ Done | Added contribution and collaboration guidelines for public repository usage |
| Low | ~~**`SECURITY.md`**~~ | ✅ Done | Added responsible security vulnerability disclosure process |
| Low | ~~**Rename `README.MD` → `README.md`**~~ | ✅ Done | Aligned repository with GitHub naming conventions |

## Achieved Benefits

- More professional repository structure
- Clearer contribution workflow
- Standardized pull request descriptions
- Standardized issue reporting
- Better readiness for public or external contribution
- Improved repository governance for portfolio evaluation
